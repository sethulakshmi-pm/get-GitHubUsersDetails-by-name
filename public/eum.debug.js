(function () {
  ...
  function stripSecrets(url) {
    ...
    try {
      ...
      var fragString = null;
      if (hashIndex >= 0) {
        fragString = url.substring(hashIndex);
      ...
      if (queryIndex >= 0) {
        var queryString = url.substring(queryIndex).split('&').map(function (param) {
  ...
  Trie.prototype.toJs = function toJs(node) {
    ...
    for (var i = 0, length = keys.length; i < length; i++) {
      var _key = keys[i];
      if (_key === INTERNAL_END_MARKER) {
        result['$'] = node[INTERNAL_END_MARKER].slice();
        continue;
      }
      var combinedKeys = _key;
      var child = node[_key];
  ...
  function getEntriesTransferFormat(performanceEntries, minStartTime) {
    ...
    for (var i = 0, len = performanceEntries.length; i < len; i++) {
      ...
      if (initiatorType !== 'xmlhttprequest' && initiatorType !== 'fetch' || entry['startTime'] < defaultVars.highResTimestampReference) {
  ...
  function transmit() {
    ...
    if (defaultVars.reportingBackends && defaultVars.reportingBackends.length > 0) {
      for (var i = 0, len = defaultVars.reportingBackends.length; i < len; i++) {
        var reportingBackend = defaultVars.reportingBackends[i];
        if (i > 0) {
          for (var j = 0, length = pendingBeacons.length; j < length; j++) {
            var beacon = pendingBeacons[j];
            beacon['k'] = reportingBackend['key'];
          }
        }
  ...
  function sendBeacon$2(data) {
    if (defaultVars.reportingBackends && defaultVars.reportingBackends.length > 0) {
      for (var i = 0, len = defaultVars.reportingBackends.length; i < len; i++) {
        ...
        if (str.length != 0) {
          transmit$1(str, reportingBackend['reportingUrl']);
        }
      }
    } else {
      var _str = stringify(data);
      if (_str.length != 0) {
        transmit$1(_str, String(defaultVars.reportingUrl));
      }
    }
  }
  ...
  var seenErrors = {};
  var scheduledTransmissionTimeoutHandle;
  ...
  function observeResourcePerformance(opts) {
    ...
    function onResource(list) {
      ...
      for (var i = 0; i < entries.length; i++) {
        var entry = entries[i];
        if (entry.startTime >= startTime && (!endTime || endTime + opts.maxToleranceForResourceTimingsMillis >= entry.responseEnd) && opts.resourceMatcher(entry)) {
          resource = entry;
  ...
  var maximumHttpRequestUrlLength = 4096;
  ...
  function captureHttpHeaders(beacon, headerString) {
    ...
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        ...
        if (matchesAny(defaultVars.headersToCapture, items[0])) {
  ...
  function processReportingBackends(arr) {
    ...
    for (var i = 0, len = arr.length; i < len; i++) {
      ...
      if (!item || !item['reportingUrl'] || !hasOwnProperty(item, 'key')) {
        ...
        defaultVars.reportingBackends.push(arr[i]);
      }
    }
  }
